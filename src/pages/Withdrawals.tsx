...
const WithdrawalsPage = () => {
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(`Withdrawal initiated! ID: ${response.data.withdrawalId}`);
      setAmount('');
      setAddress('');
    } catch (err: any) {
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-slate-400 mt-1">Transfer money out of your account</p>
      </div>
        ...
        <form onSubmit={handleWithdraw} className="space-y-6">
          <div>
            ...
            <div className="space-y-2">
              {[
                { value: 'bitcoin', label: '₿ Bitcoin Wallet' },
          ...
          {method === 'bitcoin' && (
            <>
              <div>
                <label className="block text-sm font-medium mb-2">Bitcoin Network</label>
                <select
                  value={network}
                  onChange={(e) => setNetwork(e.target.value)}
                  className="w-full px-4 py-2 rounded-lg bg-white/5 border border-white/10 focus:border-orange-400 focus:outline-none text-white"
                >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="bc1..."
                  className="w-full px-4 py-2 rounded-lg bg-white/5 border border-white/10 focus:border-orange-400 focus:outline-none text-white placeholder-slate-500 font-mono text-sm"
                />
              </div>
            </>
          )}
          ...
          <div className="p-4 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-yellow-200 text-sm">
            ⚠️ Withdrawals are subject to review. Please allow 1-2 business days for processing.
          </div>
