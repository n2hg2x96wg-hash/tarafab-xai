...
const TransactionsPage = () => {
  ...
  if (loading) return <div className="animate-shimmer w-full h-96 rounded-xl" />;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <input type="text" placeholder="Search transactions..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 px-4 py-2 rounded-lg bg-white/5 border border-white/10 focus:border-orange-400 focus:outline-none text-white placeholder-slate-500" />
        <select value={filter} onChange={(e) => setFilter(e.target.value)} className="px-4 py-2 rounded-lg bg-white/5 border border-white/10 focus:border-orange-400 focus:outline-none text-white">
          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredTransactions.length > 0 ? filteredTransactions.map((tx) => {
                const outgoing = tx.type.includes('transfer_out') || tx.type === 'withdrawal';
                return <tr key={tx.id} className="border-b border-white/5 hover:bg-white/5 transition"><td className="py-4 px-6 capitalize">{tx.type.replace(/_/g, ' ')}</td><td className="py-4 px-6 font-medium"><span className={outgoing ? 'text-red-400' : 'text-green-400'}>{outgoing ? '-' : '+'}${tx.amount.toLocaleString('en-US', { maximumFractionDigits: 2 })}</span></td><td className="py-4 px-6 text-slate-400">${tx.fee.toLocaleString('en-US', { maximumFractionDigits: 2 })}</td><td className="py-4 px-6"><span className={`px-3 py-1 rounded-full text-xs font-medium ${tx.status === 'completed' ? 'bg-green-500/10 text-green-400' : tx.status === 'rejected' ? 'bg-red-500/10 text-red-400' : 'bg-yellow-500/10 text-yellow-400'}`}>{tx.status}</span></td><td className="py-4 px-6 text-slate-400 text-sm">{new Date(tx.timestamp).toLocaleString()}</td></tr>;
