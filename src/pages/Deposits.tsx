...
const DepositsPage = () => {
  ...
  const handleInitiateDeposit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/api/deposits/initiate', { method, amount: parseFloat(amount), asset: method === 'bitcoin' ? 'BTC' : 'USD' });
      setDepositDetails(response.data); setDepositId(response.data.depositId);
    } catch (err: any) {
  ...
  const handleSubmitHash = async () => {
    if (!transactionHash || !depositId) return setError('Please enter a transaction hash');
    setLoading(true); setError('');
    try {
      await api.post('/api/deposits/verify-transaction', { depositId, transactionHash });
  ...
  return (
    <div className="space-y-8">
      <div><h1 className="text-3xl font-bold">Deposit Funds</h1><p className="text-slate-400 mt-1">Add money to your account</p></div>
      <div className="grid md:grid-cols-2 gap-8">
        <div className="glass rounded-xl p-8">
          ...
          <form onSubmit={handleInitiateDeposit} className="space-y-4">
            <div><label className="block text-sm font-medium mb-3">Payment Method</label>
              <div className="space-y-2">{[
                { value: 'bitcoin', label: '₿ Bitcoin' }, { value: 'bank_transfer', label: '🏦 Bank Transfer' }, { value: 'card', label: '💳 Credit Card' },
              ].map((opt) => <label key={opt.value} className="flex items-center p-3 rounded-lg border border-white/10 cursor-pointer hover:border-orange-400/50 transition">
        ...
        {depositDetails && <div className="glass rounded-xl p-8">
          ...
          {method === 'bitcoin' && <div className="space-y-6">
            <div><p className="text-slate-400 text-sm mb-2">Bitcoin Address</p><div className="p-4 rounded-lg bg-white/5 border border-white/10 font-mono text-sm break-all">{depositDetails.address}</div><button onClick={() => { navigator.clipboard.writeText(depositDetails.address); alert('Address copied to clipboard'); }} className="mt-2 text-sm text-orange-400">Copy Address</button></div>
            <div><p className="text-slate-400 text-sm mb-2">Amount (BTC)</p><p className="text-2xl font-bold">{depositDetails.amount}</p></div>
            <div><p className="text-slate-400 text-sm mb-2">Network</p><p className="text-white capitalize">{depositDetails.network}</p></div>
            <div><label className="block text-sm font-medium mb-2">Transaction Hash</label><div className="flex gap-2"><input type="text" value={transactionHash} onChange={(e) => setTransactionHash(e.target.value)} placeholder="Paste transaction hash (txid)" className="flex-1 px-4 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm" /><button onClick={handleSubmitHash} disabled={loading || !transactionHash} className="px-4 py-2 rounded-lg bg-orange-500/20 text-orange-400 disabled:opacity-50">Verify</button></div></div>
            <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-200 text-sm">Send the exact amount to the address above. Confirmation is handled by the backend verification flow.</div>
          </div>}
